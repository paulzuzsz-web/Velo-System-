...
function buildShort(video) {
  ...
  likeBtn.innerHTML = `${liked ? '❤️' : '🤍'}<span class="count">${video.likes.length}</span>`;
  likeBtn.addEventListener('click', async () => {
    const i = video.likes.indexOf(me);
    if (i >= 0) video.likes.splice(i, 1);
    else video.likes.push(me);
    await dbPutVideo(video);
    const nowLiked = video.likes.includes(me);
    likeBtn.classList.toggle('liked', nowLiked);
    likeBtn.innerHTML = `${nowLiked ? '❤️' : '🤍'}<span class="count">${video.likes.length}</span>`;
  });

  const commentBtn = document.createElement('button');
  commentBtn.className = 'action-btn';
  commentBtn.innerHTML = `💬<span class="count">${video.comments.length}</span>`;
...
function buildGridItem(video, { deletable = false } = {}) {
  ...
  likes.textContent = '❤️ ' + video.likes.length;
  ...
  if (deletable) {
    ...
    del.className = 'grid-likes';
    del.style.cssText = 'top:6px;left:6px;right:auto;border:none;color:#fff;cursor:pointer';
    del.textContent = '🗑️';
...
async function renderProfile(username) {
  ...
  if (videos.length === 0) {
    ...
    empty.innerHTML = '<p>📭</p><p>Noch keine Videos hochgeladen.</p>';
